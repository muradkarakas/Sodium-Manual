...
#### Description
 ...
 Just before sending content of the data block row to the client, post\_query trigger is fired for each row.  
 `post_query` trigger function is optional. 

if it is needed, must be in [Code Behind File](../program-structure/code-behind-file.md).  
  
`post_query` trigger is a function with a specific name. Trigger function and its name should conform to the following format:
